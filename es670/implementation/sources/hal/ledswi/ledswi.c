...
status_type_e ledswi_getSwitchStatus(char cSwitchNum)
{
    ...
    if(cSwitchNum <= MAX_LED_SWI)
    {
        ...
        {
            case 1:
                if(ON == ((GPIOA_PDIR & LS1_DIR_INPUT) >> LS1_PIN) )
                    sstReturn = ON;
                break;

            case 2:
                if(ON == ((GPIOA_PDIR & LS2_DIR_INPUT) >> LS2_PIN) )
                    sstReturn = ON;
                break;

            case 3:
                if(ON == ((GPIOA_PDIR & LS3_DIR_INPUT) >> LS3_PIN) )
                    sstReturn = ON;
                break;

            case 4:
                if(ON == ((GPIOA_PDIR & LS4_DIR_INPUT) >> LS4_PIN) )
                    sstReturn = ON;
                break;
...
status_type_e ledswi_getLedStatus(char cLedNum)
{
    ...
    if(cLedNum <= MAX_LED_SWI)
    {
        ...
        {
            case 1:
                if(ON == ((GPIOA_PDOR & LS1_DIR_OUTPUT) >> LS1_PIN) )
                    sstReturn = ON;
                break;

            case 2:
                if(ON == ((GPIOA_PDOR & LS2_DIR_OUTPUT) >> LS2_PIN) )
                    sstReturn = ON;
                break;

            case 3:
                if(ON == ((GPIOA_PDOR & LS3_DIR_OUTPUT) >> LS3_PIN) )
                    sstReturn = ON;
                break;

            case 4:
                if(ON == ((GPIOA_PDOR & LS4_DIR_OUTPUT) >> LS4_PIN) )
                    sstReturn = ON;
                break;
